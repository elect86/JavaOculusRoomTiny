...
public class OculusRoomModel {
    ...
    private static byte[] getCheckerTexture() {
        ...
        Vec4 black = new Vec4(80f, 80f, 80f, 255f);

        Vec4 white = new Vec4(180f, 180f, 180f, 255f);
        ...
        for (int j = 0; j < 256; j++) {
            ...
            for (int i = 0; i < 256; i++) {
//                int a = (((i / 4 >> 5) ^ (j / 4 >> 5)) & 1);
                Vec4 color = (((i / 4 >> 5) ^ (j / 4 >> 5)) & 1) == 1 ? white : black;

                checker[(j * 256 + i) * 4] = (byte) color.x;
                checker[(j * 256 + i) * 4 + 1] = (byte) color.x;
                checker[(j * 256 + i) * 4 + 2] = (byte) color.x;
                checker[(j * 256 + i) * 4 + 3] = (byte) color.x;
            }
        }
        return checker;
    }
